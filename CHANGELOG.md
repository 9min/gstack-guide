...
- gstack v1.1.1.0 동기화 — 신규 스킬 `/gstack-plan-tune` 가이드 추가 (질문 학습 + 5차원 개발자 프로필 + 8가지 아키타입 + 관찰 모드 v1)
- 신규 카테고리 **"개인화"** 신설 — 향후 개인화 스킬 확장 대비, plan-tune이 첫 항목
- `/gstack-browse` v1.1.0.0 기능 반영 — `load-html`, `file://` 네비게이션, `--selector`/`--scale` 플래그, 명령어 별칭(setcontent), "Did you mean..?" 타이포 힌트, 상태 파일 주입 방어 문서화
- `/gstack-ship` v1.1.1.0 반영 — VERSION ↔ package.json 드리프트 4-state 감지(FRESH/ALREADY_BUMPED/DRIFT_STALE_PKG/DRIFT_UNEXPECTED)
- `/gstack-codex` v0.18.4.0 반영 — Apple Silicon 바이너리 자동 서명, 타임아웃/인증 사전 점검
- `/gstack-upgrade` — v1.0.0.0 문체 전환 첫 실행 프롬프트 안내 (`explain_level default|terse`)
- `/gstack-retro` v1.0.0.0 반영 — 출시 기능/커밋/PR을 SLOC보다 우선 표시하는 헤드라인 순서

### Changed

- footer 버전 `v0.16.3.0 → v1.1.1.0` 자동 동기화 (scripts/sync.js가 `~/.claude/skills/gstack/VERSION`에서 읽음)
- `package.json` 버전 `1.0.2 → 1.1.0` (신규 스킬 + 신규 카테고리로 MINOR 범프)
