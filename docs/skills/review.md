...
리뷰는 여러 전문가 스페셜리스트(testing, maintainability, security, performance, data-migration, api-contract, design, red-team)를 병렬로 디스패치합니다. 각 스페셜리스트는 자신의 전문 영역만 집중 검토하고, 테스트 스텁 제안이 포함된 경우 ASK 항목으로 표시됩니다.

**리뷰 학습 기능** — 스킵한 이슈는 기억됩니다. 다음 리뷰에서 동일한 패턴의 이슈가 나오면, 관련 코드가 변경되지 않은 한 다시 보고하지 않습니다. 같은 이슈를 반복해서 스킵할 필요가 없어집니다.

**적응형 스페셜리스트 게이팅** — 10회 이상 디스패치했는데 한 번도 이슈를 찾지 못한 스페셜리스트는 자동으로 게이팅됩니다. 보안(security), 데이터 마이그레이션(data-migration)은 항상 실행됩니다. 특정 스페셜리스트를 강제 실행하려면 `--security`, `--performance` 등 플래그를 사용합니다.
