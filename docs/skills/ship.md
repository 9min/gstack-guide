...
**VERSION ↔ package.json 드리프트 감지** — v1.1.1.0부터 Step 12가 두 버전 파일이 어긋날 가능성을 네 가지 상태(FRESH, ALREADY_BUMPED, DRIFT_STALE_PKG, DRIFT_UNEXPECTED)로 분류합니다. `VERSION`만 올리고 `package.json`이 구버전으로 남아 있으면 sync-only 경로로 자동 복구되고, 두 값이 이해할 수 없는 방향으로 어긋나면 작업을 멈추고 명확히 알립니다. `NEW_VERSION`은 기록 전 4자리 semver 패턴으로 검증되고, `package.json`이 비정상 JSON이면 조용히 덮어쓰지 않고 중단합니다.
